...
static __INLINE uint8_t wireless_reg_read(uint8_t reg)
{
  ...
  wireless_CSN_low();
	...
	spiTx(RF_SPI_BASE, &tx[0], 1, &rx[0]);
	spiTx(RF_SPI_BASE, &tx[1], 1, &rx[1]);
...
{
	uint8_t tx = 0x20 | 0x10;
	uint8_t rx;
	
  wireless_CSN_low();
	
	spiTx(RF_SPI_BASE, &tx, 1, &rx);
	spiTx(RF_SPI_BASE, &tx_addr[0], 1, &rx);
	spiTx(RF_SPI_BASE, &tx_addr[1], 1, &rx);
	spiTx(RF_SPI_BASE, &tx_addr[2], 1, &rx);
	spiTx(RF_SPI_BASE, &tx_addr[3], 1, &rx);
	spiTx(RF_SPI_BASE, &tx_addr[4], 1, &rx);
...
static __INLINE void wireless_tx_data_payload( uint32_t data)
{
  ...
  wireless_CSN_low();
	...
	spiTx(RF_SPI_BASE, &tx[0], 1, &rx);
	spiTx(RF_SPI_BASE, &tx[1], 1, &rx);
	spiTx(RF_SPI_BASE, &tx[2], 1, &rx);
	spiTx(RF_SPI_BASE, &tx[3], 1, &rx);
	spiTx(RF_SPI_BASE, &tx[4], 1, &rx);
